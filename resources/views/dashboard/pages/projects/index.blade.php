                                        ...
                                        @foreach ($items as $data)
                                            <tr>
                                                ...
                                                <td><img style="width: 90px; height: 90px;"
                                                        src="{{ asset('images/Project/' . $data->images()->first()->image) }}">
                                                </td>
                                                <td>{{ $data->translate($key)->title }}</td>
                                                <td style="word-break: break-all;">
                                                    {{ $data->translate($key)->description }}</td>

                                                <td>
                                                    <a target="_blank" href="{{ asset('pdfs/Project/' . $data->pdf) }}">
                                                ...
                                                <td>
                                                    <div class="hidden-sm hidden-xs action-buttons" style="display:flex;">
                                                        ...
                                                        <a style="color:blue"
                                                            href="{{ route('projects.show', $data->id) }}">
                                                            <i class="dripicons-zoom-in bigger-120"
                                                                style="margin-right: 10px;"></i>
                                                        </a>
                                                        <form method="POST"
                                                            action="{{ route('projects.destroy', $data->id) }}">
                                                            @csrf
                                                            @method('DELETE')
