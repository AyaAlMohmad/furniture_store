                        ...
                        @foreach (config('app.languages') as $key => $lang)
                            ...
                            <div class="tab-pane {{ $lang == 'English' ? 'active' : '' }}" id="{{ $key }}">
                                ...
                                <table id="datatable-buttons"
                                    ...
                                    <thead>
                                        <tr>
                                            <th >{{ __('dashboard.date', [], $key) }}</th>
                                            <th >{{ __('dashboard.title', [], $key) }}</th>
                                            <th >{{ __('dashboard.description', [], $key) }}</th>
                                            <th >{{ __('dashboard.short_description', [], $key) }}</th>
                                    ...
                                    <tbody>
                                        @foreach ($items as $data)
                                            <tr>
                                                ...
                                                <td style="word-break: break-all;">{{ $data->translate($key)->short_description }}</td>

                                                <td>
                                                    <div class="hidden-sm hidden-xs action-buttons" style="display:flex;">
                                                        <a style="color:green" href="{{ route('investors.edit', $data->id) }}">
                                                            <i class="fas fa-feather bigger-120"
                                                                style="margin-right: 10px;"> </i>
                                                        </a>
                                                        <a style="color:blue" href="{{ route('investors.show', $data->id) }}">
                                                            <i class="dripicons-zoom-in bigger-120"
                                                                style="margin-right: 10px;"></i>
                                                        </a>
                                                        <form method="POST"
                                                            action="{{ route('investors.destroy', $data->id) }}">
                                                            @csrf
                                                            @method('DELETE')
