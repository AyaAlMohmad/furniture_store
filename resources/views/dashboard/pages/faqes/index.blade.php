                        ...
                        @foreach (config('app.languages') as $key => $lang)
                            ...
                            <div class="tab-pane {{ $lang == 'English' ? 'active' : '' }}" id="{{ $key }}">
                                ...
                                <table id="datatable-buttons"
                                    ...
                                    <thead>
                                        <tr>
                                            <th >{{ __('dashboard.question', [], $key) }}</th>
                                            <th >{{ __('dashboard.answer', [], $key) }}</th>
                                    ...
                                    <tbody>
                                        @foreach ($items as $data)
                                            <tr>
                                                ...
                                                <td style="word-break: break-all;">{{ $data->translate($key)->question }}</td>
                                                <td style="word-break: break-all;">{{ $data->translate($key)->answer }}</td>

                                                <td>
                                                    <div class="hidden-sm hidden-xs action-buttons" style="display:flex;">
                                                        <a style="color:green" href="{{ route('faqes.edit', $data->id) }}">
                                                            <i class="fas fa-feather bigger-120"
                                                                style="margin-right: 10px;"> </i>
                                                        </a>
                                                        <a style="color:blue" href="{{ route('faqes.show', $data->id) }}">
                                                            <i class="dripicons-zoom-in bigger-120"
                                                                style="margin-right: 10px;"></i>
                                                        </a>
                                                        <form method="POST"
                                                            action="{{ route('faqes.destroy', $data->id) }}">
                                                            @csrf
                                                            @method('DELETE')
